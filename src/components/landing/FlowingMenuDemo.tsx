...
import '@/app/FlowingMenuDemo.css';
...
const FlowingMenuDemo: React.FC<FlowingMenuProps> = ({ items = demoItems }) => {
  return (
    <div className="menu-wrap-demo">
      <nav className="menu-demo">
        {items.map((item, idx) => (
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        <span>{text}</span>
        <div className="marquee__img-demo" style={{ backgroundImage: `url(${image})` }} />
      </React.Fragment>
    ));
  }, [text, image]);
    ...
    <div className="menu__item-demo" ref={itemRef}>
      ...
      <div className="marquee-demo" ref={marqueeRef}>
        <div className="marquee__inner-wrap-demo" ref={marqueeInnerRef}>
          <div className="marquee__inner-demo" aria-hidden="true">
